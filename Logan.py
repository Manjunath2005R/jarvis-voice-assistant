...
def speak(text, emotion=None):
    ...
    try:
            requests.post("http://localhost:5000/api/logan_output", json={"response": text})
...
def save_memory():
    with open(MEMORY_FILE, "w") as file:
        json.dump(logan_memory, file)

logan_memory = load_memory()
logan_memory.setdefault("usage_stats", {})
last_suggestion = logan_memory.get("last_suggestion", {})

def update_usage_stat(command_key):
    now = datetime.now()
    hour_block = f"{now.hour // 4 * 4:02d}-{(now.hour // 4 + 1) * 4 - 1:02d}"
    usage = logan_memory["usage_stats"].setdefault(command_key, {})
...
def update_usage_stat(command_key):
    ...
    usage_all = logan_memory.setdefault("usage_stats", {})
    usage_today = usage_all.setdefault(today_str, {})
    usage_block = usage_today.setdefault(command_key, {})

    usage_block[hour_block] = usage_block.get(hour_block, 0) + 1
    save_memory()
...
def suggest_frequent_app():
    ...
    usage_today = logan_memory.get("usage_stats", {}).get(today_str, {})
    most_used = None
    max_count = 0

    for app, times in usage_today.items():
        if times.get(current_block, 0) > max_count:
            most_used = app
            max_count = times[current_block]

    # Only suggest if used 2+ times AND not suggested today
    if most_used and max_count >= 2:
        last_suggest_date = last_suggestion.get(most_used)
        if last_suggest_date == today_str:
            return  # Already suggested today, skip
        ...
        try:
            ...
            # Save suggestion date so it doesn't repeat today
            last_suggestion[most_used] = today_str
            logan_memory["last_suggestion"] = last_suggestion
...
def interpret(command):
    ...
    if 'your name' in command:
        speak("My name is logan, your personal assistant.", emotion="gentle")
        ...
        try:
            topic = command.replace("logan", "").replace("what is", "").replace("what's", "").replace("who is", "").replace("tell me about", "").strip()
        ...
        if " is " in key_value:
            ...
            logan_memory[key.strip()] = value.strip()
...
def logan_listener():
    while True:
        ...
        if "logan" in full_query:
            cleaned_command = full_query.replace("logan", "").strip()
            if cleaned_command:
                print("[Wake+Command Mode] Detected full command with 'logan'")
...
def run_logan():
    ...
    print("logan Listener Started (running in foreground for debugging)")
    logan_listener()
